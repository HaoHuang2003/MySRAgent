...
def build_splits( # 已经过人类审核，不许修改这个函数的接口和行为
    node_idx: np.ndarray,
    time_idx: np.ndarray,
    seed: int,
) -> Dict[str, tuple[np.ndarray, np.ndarray]]:
    rng = np.random.default_rng(seed)
    shuffled = np.array(node_idx, copy=True)
    rng.shuffle(shuffled)
    cut = len(shuffled) // 2
    if cut == 0 or cut == len(shuffled):
        raise ValueError(f"Cannot split {len(shuffled)} values into two non-empty halves.")
    node_a = np.sort(shuffled[:cut])
    node_b = np.sort(shuffled[cut:])

    time_cut = len(time_idx) // 2
    if time_cut == 0 or time_cut == len(time_idx):
        raise ValueError(f"Cannot split {len(time_idx)} time steps into two non-empty halves.")
    time_a = np.sort(time_idx[:time_cut])
    time_b = np.sort(time_idx[time_cut:])

    return {
        "train": (node_a, time_a),
        "eval_other_points": (node_b, time_a),
        "eval_other_times": (node_a, time_b),
        "eval_other_points_and_times": (node_b, time_b),
    }
